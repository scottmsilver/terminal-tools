...
def get_workspace_name(windows, workspace_num, repo_names):
    ...
    for model in DEFAULT_MODELS:
        try:
            payload = {
                "model": model, "prompt": prompt, "images": [screenshot_b64] if screenshot_b64 else [],
                "stream": False, "options": {"temperature": 0.1, "num_ctx": 32768}
            }
            # Save debug payload
